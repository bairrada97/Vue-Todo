...
<div class="home">
  ...
  <ul>
    <Item v-for="(todos, index) in todoList" :index="index" :todos="todos" :deleteItem="deleteItem" :completedTask="completedTask" />
  </ul>
  <div class="completedContainer">
    <h2>{{ msgCompleted }}</h2>
    <ul class="">
      <CompletedList v-for="completed in completedList" :completed="completed" />
    </ul>
  </div>
</div>
</template>
